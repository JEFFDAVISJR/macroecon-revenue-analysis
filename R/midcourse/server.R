...
function(input, output, session) {
  ...
  # third plot: scatter plot with dynamic X and dynamic Y-axis
  output$scatterPlot <- renderPlot({
    ...
    # 2. Merge month_rev with non_gdp using the standardized 'Year-Month'
    merged_data <- non_gdp %>%
      left_join(month_rev, by = "Year-Month", suffix = c("", "_month_rev"))  # makes a unique suffix for Total_Rev
    
    # 3. merge for Total_Rev_Offset1, Total_Rev_Offset2
    merged_data <- merged_data %>%
      left_join(month_rev, by = c("Year-Month_Offset1" = "Year-Month"), suffix = c("", "_Offset1"))  # makes it unique
    merged_data <- merged_data %>%
      left_join(month_rev, by = c("Year-Month_Offset2" = "Year-Month"), suffix = c("", "_Offset2"))  # makes it unique
    
    # 4. get selected variable for the x-axis
    x_var <- input$scatter_x_var
    
    # 5. get selected variable for the y-axis
    y_var <- input$scatter_y_var
    
    # 6. plot the selected X and Y axis variables
